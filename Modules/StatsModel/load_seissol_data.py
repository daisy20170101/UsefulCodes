...
def load_rake_data(datafile):
    ...
    return rk_max, rk_min, rk_avg, rk_50th, u_avg, subdata
...
def load_ss_slp_area_data(ssfile):
    ...
    # radiated energy
    rad_eng = np.sum(
        0.5 * drop[index] * df2["ASl"].to_numpy()[index] * df2["Area"].to_numpy()[index]
    ) - np.sum(0.5 * drop[index] * dc_mod * df2["Area"].to_numpy()[index])
    rad_eng_mean = np.mean(
        0.5 * drop[index] * df2["ASl"].to_numpy()[index] - (0.5 * drop[index] * dc_mod)
    )
