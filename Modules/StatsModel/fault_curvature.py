...
def compute_principal_curvatures(
    vertices: np.ndarray, faces: np.ndarray, method: str = "meyer"
...
import importlib, StatsModel.mesh_func4

importlib.reload(StatsModel.mesh_func4)
from StatsModel.mesh_func4 import (
    find_neighbors,
...
def discrete_mean_curvature(vertex_idx, vertices, faces, method="uniform"):
    """
    Robust version with validation, multiple methods, and error handling
    """
    # Input validation
    ...
    if len(neighbors) < 2:
        warnings.warn(f"Vertex {vertex_idx} has insufficient neighbors")
        return 0.0
    ...
    if method == "uniform":
        ...
        for neighbor_idx in neighbors:
            laplacian += vertices[neighbor_idx] - center
        laplacian /= len(neighbors)

    elif method == "cotangent":
        # Cotangent weights with fallback to uniform
        # (see full implementation for details)
        pass

    return 0.5 * np.linalg.norm(laplacian)


def compute_mesh_curvatures(vertices, faces, method="uniform", validate=True):
    """
    SOLVES YOUR CONCERN: No longer assumes loop index = vertex index
    """
    ...
    if validate:
        ...
        if not validation_info["is_valid"]:
            raise ValueError(f"Invalid mesh: {validation_info['invalid_indices']}")

    # KEY CHANGE: Find vertices actually used in faces
    used_vertex_indices = set()
    for face in faces:
        used_vertex_indices.update(face)  # Get actual vertex indices

    curvatures = {}

    # Process only vertices that appear in faces (not loop over range!)
    for vertex_idx in used_vertex_indices:  # vertex_idx is guaranteed valid
        curvature_info = {"H": 0.0, "K": 0.0, "valid": False, "error": None}

        try:
            H = discrete_mean_curvature(vertex_idx, vertices, faces, method)
            K = discrete_gaussian_curvature(vertex_idx, vertices, faces)
            curvature_info.update({"H": H, "K": K, "valid": True})
