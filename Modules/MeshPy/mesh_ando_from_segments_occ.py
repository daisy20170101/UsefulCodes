...
def generate_mesh_from_fault_segments(segment_files,
    ...
    all_fault_surfaces= gmsh.model.getEntities(dim=2)

    obstacles = []
    # Create a new physical group for surfaces
    for surface in all_fault_surfaces:
        obstacles.append(surface[1])

    print('fault:',obstacles)
    ...
    # Box bounds
    xmin = fault_xmin - box_length / 2 - buffer_x
    xmax = fault_xmax + box_length / 2 + buffer_x
    ymin = fault_ymin - box_width / 2 - buffer_y
    ymax = fault_ymax + box_width / 2 + buffer_y
    zmin = -box_depth
    zmax =-2000.0
    ...
    # kept, _ = gmsh.model.occ.fragment([(3, v1)], [(2, tag) for tag in [12,18,32]])
    kept, _ = gmsh.model.occ.fragment([(3, v1)], [(2, 12)])

    new_vols = [tag for (dim, tag) in kept if dim == 3]
    new_surfs = [tag for (dim, tag) in kept if dim == 2]

    print(new_vols, new_surfs)
    gmsh.model.occ.synchronize()
    ...
    for surf in all_surfaces:
        all_surf.append(surf[1])

    print('all_surf',all_surf)

    fault_surfaces = [s for s in all_surf[6::]]

    print(f"\nSurface classification:")
    print(f"  Free surface (top): {freesurf}")
    print(f"  Fault surfaces: {fault_surfaces}")
    ...
    print(f"  ✓ Added group 103 (fault): {len(fault_surfaces)} surfaces")
