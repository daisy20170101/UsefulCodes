...
def plot_gme_hik(pgv,sa1,sa3,triang,coast_table, hik_trace, faultTable,modelname='test1', imd='pgv'):
    ...
    if imd=='sa1':
        ...
        sc = ax0.tripcolor(triang,sa1[:]/9.8,cmap=Cmap2, shading='flat',vmin=0,vmax=2.)
        ...
        sc = ax0.tripcolor(triang,sa3[:]/9.8,cmap=Cmap2, shading='flat',vmin=0,vmax=1.5)
        cl = fig.colorbar(sc,ax=ax0)
        ...
        sc = ax0.tripcolor(triang,pgv[:],cmap=Cmap2, shading='flat',vmin=0,vmax=2.5)
        cl = fig.colorbar(sc,ax=ax0)
...
def plot_gme_crust(pgv,sa1,sa3,triang,coast_table, faultTable,modelname='test1',  imd='pgv'):
    ...
    if imd=='sa1':
        print('plotting SA1.0s')
        sc = ax0.tripcolor(triang,sa1[:]/9.8,cmap=Cmap2, shading='flat',vmin=0,vmax=0.5)
        cl = fig.colorbar(sc,ax=ax0)
        cl.set_label('SA1.0s (g)')
        ...
        sc = ax0.tripcolor(triang,sa3[:]/9.8,cmap=Cmap2, shading='flat',vmin=0,vmax=0.2)
        cl = fig.colorbar(sc,ax=ax0)
        cl.set_label('SA3.0s (g)')
        ...
        sc = ax0.tripcolor(triang,pgv[:],cmap=Cmap2, shading='flat',vmin=0,vmax=1.0)
        cl = fig.colorbar(sc,ax=ax0)
        cl.set_label('PGV (m/s)')
    ...
    # ax0.set_yticks(lat_ticks)
    ax0.set(xlim=( epi300[0]-10e3, epi300[0]+70e3),ylim=(epi300[1]-50e3,epi300[1]+120e3))
